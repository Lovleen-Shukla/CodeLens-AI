...
export class DashboardPanel {
    ...
    try {
      ...
      if (fs.existsSync(repoDir)) {
        const choice = await vscode.window.showWarningMessage(
          `"${repoName}" already exists in that folder.`,
      ...
      const openChoice = await vscode.window.showInformationMessage(
        `Cloned ${owner}/${repoName} successfully!`,
        'Open in this window', 'Open in new window', 'Skip'
      );
  ...
  private async analyzeGithubRepo(repoUrl: string) {
    ...
    try {
      ...
      if (!repoRes.ok) {
        if (repoRes.status === 404) { throw new Error(`Not found or private.`); }
        if (repoRes.status === 403) { throw new Error('Rate limit hit. Try again shortly.'); }
        throw new Error(`GitHub API error: ${repoRes.status}`);
      }
      const repoData = await repoRes.json() as {
        description: string; stargazers_count: number; forks_count: number;
        default_branch: string; language: string; topics: string[];
        open_issues_count: number; watchers_count: number; size: number;
        license?: { name: string }; created_at: string; updated_at: string;
        fork: boolean; parent?: { full_name: string };
        allow_forking: boolean; visibility: string;
      };
      ...
      let readmeContent = '';
      try { const rr = await fetch(`${apiBase}/readme`, { headers: { ...headers, 'Accept': 'application/vnd.github.v3.raw' } }); if (rr.ok) { readmeContent = (await rr.text()).slice(0, 5000); } } catch { /* */ }

      let fileTree = ''; let totalFiles = 0;
      try {
        const tr = await fetch(`${apiBase}/git/trees/${repoData.default_branch}?recursive=1`, { headers });
        if (tr.ok) {
          const td = await tr.json() as { tree: Array<{ path: string; type: string }> };
      ...
      for (const df of ['package.json', 'requirements.txt', 'go.mod', 'Cargo.toml']) {
        try { const dr = await fetch(`${apiBase}/contents/${df}`, { headers: { ...headers, 'Accept': 'application/vnd.github.v3.raw' } }); if (dr.ok) { depsContent = `--- ${df} ---\n${(await dr.text()).slice(0, 1000)}`; break; } } catch { /* */ }
      }
...
## Verdict`;
  ...
  private async generateArchDiagram() {
    ...
    try {
      const result = await this._ai.ask(
        `Project Stack: ${stack}\nFile Tree:\n${tree}`,
        `You are a Senior Software Architect. Analyze this project and create an architecture diagram JSON.
  ...
  "connections": [
    { "from": "file.ts", "to": "other.ts", "label": "imports" }
      ...
      try {
        ...
        if (jsonMatch) { parsed = JSON.parse(jsonMatch[0].replace(/,(\s*[\]}])/g, '$1')); }
      } catch (e) { console.error('Arch parse error:', e); }
