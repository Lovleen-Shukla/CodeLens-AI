  ...
  private async explainFile(relPath: string) {
    ...
    try {
      const explanation = await this._ai.ask(
        `Explain this file:\n\nFile: ${relPath}\nImports:\n${imports}\n\nContents:\n${content}`,
        `You are CodeLens AI. Explain code files clearly. Format your response as:
...
private async generateArchDiagram() {
  ...
  try {
    const result = await this._ai.ask(
      `Project Stack: ${stack}\nFile Tree:\n${tree}`,
      `You are a Senior Software Architect. Create a layered architecture diagram JSON.
       ...
       JSON Shape: { "title": "...", "layers": [{ "name": "...", "color": "#hex", "nodes": ["file.js"] }], "connections": [{ "from": "file.js", "to": "other.js", "label": "imports" }] }`,
      4096 
    );
