...
export class DashboardPanel {
  ...
  private constructor(panel: vscode.WebviewPanel, context: vscode.ExtensionContext, ai: AIClient) {
    ...
    this._panel.webview.onDidReceiveMessage(async (msg) => {
      switch (msg.type) {
        ...
        case 'saveArchPng': await this.saveArchPng(msg.pngBase64, msg.suggestedName); break;
  ...
  private runCmd(cmd: string): Promise<string> {
    return new Promise((res, rej) => {
      child_process.exec(cmd, { timeout: 120000 }, (err: child_process.ExecException | null, stdout: string, stderr: string) => {
        if (err) { rej(new Error(stderr || err.message)); } else { res(stdout); }
      });
    });
  }

  private async saveArchPng(pngBase64: string, suggestedName?: string) {
    try {
...
- node type: entry=entrypoint, service=logic, util=helper, model=data, config=config, test=test, style=css`;
      ...
      const repairJsonLike = (s: string) => sanitizeJson(s)
      ...
      if (!parsed && !jsonCandidate) { parseError = 'No JSON found in response.'; }
      this._panel.webview.postMessage({
