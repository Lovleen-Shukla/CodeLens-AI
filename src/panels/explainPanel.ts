...
export class ExplainPanel {
  static currentPanel?: ExplainPanel;
  private readonly _panel: vscode.WebviewPanel;

  static show(context: vscode.ExtensionContext, title: string, code: string, lang: string): ExplainPanel {
  ...
  private constructor(panel: vscode.WebviewPanel, title: string, code: string, lang: string) {
    this._panel = panel;
    this._panel.webview.html = this._buildHtml(title, code, lang);
  }

  private _reset(title: string, code: string, lang: string) {
    this._panel.webview.html = this._buildHtml(title, code, lang);
  }
  ...
  private _buildHtml(title: string, code: string, lang: string): string {
    ...
    // Loading skeletons
    const skels = [80, 95, 70, 88, 60, 92, 75, 50].map(w =>
