...
class DashboardPanel {
    ...
    runCmd(cmd) {
        return new Promise((res, rej) => { child_process.exec(cmd, { timeout: 120000 }, (err, stdout, stderr) => { if (err) {
    ...
    async generateArchDiagram() {
        ...
        try {
            ...
            if (!parsed) {
                try {
                    const mx = result.match(/\{[\s\S]*\}/);
                    if (mx) {
                        parsed = JSON.parse(mx[0].replace(/,(\s*[}\]])/g, '$1').replace(/[\u0000-\u001F]/g, ' '));
                    }
                    else {
                    ...
                    parseError = String(e2).slice(0, 100);
                }
            }
