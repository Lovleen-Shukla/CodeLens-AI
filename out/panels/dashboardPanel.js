    ...
    async analyzeGithubRepo(repoUrl) {
        ...
        try {
            ...
            try {
                ...
                if (rr.ok) {
                    readmeContent = (await rr.text()).slice(0, 4000);
                }
            }
            catch { /* */ }
            let fileTree = '';
            try {
                ...
                if (tr.ok) {
                    ...
                    fileTree = td.tree.filter(i => i.type === 'blob').map(i => i.path).filter(p => !p.includes('node_modules') && !p.includes('.git')).slice(0, 80).join('\n');
                }
            }
            catch { /* */ }
            const keyFiles = [];
